...
5. In the Supabase dashboard: Authentication → Sign In / Providers → enable **Anonymous sign-ins** (done for project `flnfwoenhylogdwskwrh`). Add `feed://auth/callback` under URL Configuration. Email sign-in uses the default magic-link template (a link that opens the app); to send 6-digit codes instead, set up custom SMTP and add `{{ .Token }}` to the "Magic link or OTP" and "Change email address" templates.
